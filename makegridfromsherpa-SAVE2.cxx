 ...
 * EXAMPLE Execution:
 *                   ./makegridfromsherpa
 *                   ./makegridfromsherpa <steeringfilepath> <numevents>
 *                   ./makegridfromsherpa steering/atlas2012_top.txt 1000000
 */
...
*/
        ...
        for(int igrid=0; igrid<NGrid; igrid++) {
            ...
            if(histoIndex==i_RB)
            {
                if(startIndex<=i_R && endIndex>=i_B) //make sure that R and B exist before adding them!
                {
                    ...
                    htestRB[igrid]->SetName("htest_RplusB_");
                    htestRB[igrid]->SetTitle("htest_RplusB_");
    ...
    LHAPDF::initPDFSet(pdfSetName.c_str(), 0);
    ...
    //loop over and write out all histograms, including convolute histo
    string filename;
    ...
    for(int histoIndex=startIndex; histoIndex<endIndex; histoIndex++) 
    {
        ...
        for(int igrid=0; igrid<NGrid; igrid++)
        {
            ...
            cout<< " makegridfromsherpa::main: Convolute performed for type: "<<ntup_names[histoIndex]<<"(histoIndex:"<<histoIndex<<")"<<endl;


            MyData *mydata=mygrid[histoIndex]->GetMyData(igrid);
            if (!mydata) cout<<" makegridfromsherpa::main: "+ntup_names[histoIndex]+"'s mydata["<<igrid<<"] not found "<<endl;
            double yfac=mydata->GetUnitfbFactor();
            double xfac=mydata->GetUnitGeVFactor();
            cout<<" makegridfromsherpa::main: Normalise xfac= "<<xfac<<" yfac= "<<yfac<<endl;
            ...
            if ( ratio1 ) {
                //ratio->Scale(1/htestEventCount[histoIndex]);
